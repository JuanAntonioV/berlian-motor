...
type Transfers = Prisma.TransferGetPayload<{
  include: {
    Store: true;
    User: true;
    Shelf: true;
...
const columnHelper = createColumnHelper<Transfers>();
...
export const transfersColumns = [
  ...
  columnHelper.accessor('ShelfTo.name', {
    header: 'Nama rak tujuan',
    enableSorting: false,
    cell: (row) => (row.getValue() ? <CopyText value={row.getValue()} /> : '-'),
  }),
    ...
    cell: (row) => (
      <div className='flexCenter gap-4'>
        <Button variant={'outline'} size={'icon'} asChild>
          <Link
            href={`/transfer-barang/${row.row.original.invoiceNumber}`}
